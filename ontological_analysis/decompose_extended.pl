...
decompose1(S,R) :- p(Z,S,[]), % Parse proposition
	Z = [p(TNP,TVP)], TVP = vp(V),  % Get verb and noun phrases
...
decompose_np1(TNP, NewSNP, [H1,H2|T]) :- TNP = np(n(N), mod([pp(PREP,TNP2)]), ext([])),
	...
	decompose_np(TNP2, NewSNP2, T), np(NewTNP2, NewSNP2, []),
	NewTNP = np(n(N), mod([pp(PREP,NewTNP2)]), ext([])),
	reverse_np(NewTNP, NewSNP),
	% nl, write('Returning: '), write(NewSNP), nl,
...
decompose_np1(TNP, NewSNP, [H1,H2|T]) :- TNP = np(n(N), mod([ger(R,TNP2)]), ext([])),
	...
	reverse_np(NewTNP, NewSNP), % SEE IF NAME IS CORRECT
...
decompose_np1(TNP,NewSNP, T) :- TNP = np(n(N), mod(M), ext([])), M \= [],
	...
	dec_modifiers(N,M,M2,R), % Eliminating the isa relations recursively ! R pb.
	NewTNP = np(n(N), mod(M2), ext([])), % Creating the new tree.
	nsubsets(M2, SubMs), !, % write(SubMs), nl,
	reverse_np(NewTNP, NewSNP),
...
dec_modifiers(N, [H0|T0],[H1|T1], [R0|T2]) :- TNP = np(n(N), mod([H0]), ext([])),
	...
	NewTNP = np(n(N), mod([H1]), ext([])), !, % write(H1), nl, nl,
...
% ISA + modifiers is not handled for now.
...
observation(SNP1,R,SNP2,[H1,H2|T]) :- % With prepositional terms.
	...
	TrN = np(n(N), mod([pp(of,TNP2), pp(by,TNP1)|M]), ext([])),
...
observation(SNP1,R,SNP2,[H1,H2|T]) :- % With prepositional terms.
	...
	TrN = np(n(N), mod([pp(of,TNP1), pp(by,TNP2)|M]), ext([])),
...
decompose_subnp([],[],[]).
decompose_subnp([HL|TL],[NewSNP|NewT],R) :-
	np(TNP,HL,[]), decompose_np(TNP,NewSNP,R1), % RENAMING PB
	decompose_subnp(TL,NewT,R2), append(R1,R2,R).
