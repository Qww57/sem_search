...
:- ensure_loaded(insulin_onthology_lexicon).  % Loading exicon.
:- ensure_loaded(unambiguous_grammar).	 % Loading the unambiguous grammar.
...
%  read_article(R) :- setof(Y, is_sentence(Y),IDs), read_article(IDs,X),
% filter(X,R).
% read_article([],[]).
